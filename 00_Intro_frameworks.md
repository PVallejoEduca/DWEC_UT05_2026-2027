...
```js
function listaTodoElemento(id, name) {
  const item = document.createElement("li");
  const span = document.createElement("span");
  span.textContent = name;
  item.id = id;
  item.appendChild(span);
  item.appendChild(botonBorradoElemento(id));
  return item;
}

function botonBorradoElemento(id) {
...
function renderizarTodoList() {
  ...
  state.forEach((task) => {
    const item = listaTodoElemento(task.id, task.name);
